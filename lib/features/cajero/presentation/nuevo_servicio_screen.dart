...
class NuevoServicioScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<NuevoServicioScreen> createState() => _NuevoServicioScreenState();
}
...
class _NuevoServicioScreenState extends ConsumerState<NuevoServicioScreen> {
  ...
  final TextEditingController _precioServicioController = TextEditingController(text: "0");
  final TextEditingController _balanceAmountController = TextEditingController(text: "0");
  String _paymentMethod = 'efectivo'; // efectivo, tarjeta, transferencia, prepago, mixto
  String _metodoPagoAdicional = ''; // additional method if prepago is partial
  
  // Mixed payments state
  final List<Map<String, dynamic>> _pagosMixtos = []; // { 'metodo': String, 'monto': double }
  ...
  Future<void> _fetchInitialData() async {
    ...
    try {
      ...
      final responses = await Future.wait([
        client.dio.get('/cashregister/status').catchError((_) => Response(requestOptions: RequestOptions(), data: {'success': false})),
      ...
      final habitacionesNormalizadas = rawHabitaciones.map((room) {
        final name = room['nombre'] ?? room['name'] ?? 'Habitación ${room['numero'] ?? room['id_habitacion'] ?? room['id'] ?? ''}'.trim();
        return {
          ...room,
          'estado': room['estado'] ?? room['status'] ?? 0,
          'status': room['status'] ?? room['estado'] ?? 0,
          'precio': double.tryParse(room['precio']?.toString() ?? '') ?? double.tryParse(room['price']?.toString() ?? '') ?? 0.0,
          'price': double.tryParse(room['price']?.toString() ?? '') ?? double.tryParse(room['precio']?.toString() ?? '') ?? 0.0,
          'tiempo': int.tryParse(room['tiempo']?.toString() ?? '') ?? int.tryParse(room['time']?.toString() ?? '') ?? 0,
          'time': int.tryParse(room['time']?.toString() ?? '') ?? int.tryParse(room['tiempo']?.toString() ?? '') ?? 0,
          'nombre': name,
          'comision_anfitriona': double.tryParse(room['comision_anfitriona']?.toString() ?? '') ?? 0.0,
        };
      }).toList();
      ...
      if (!hasOpenCaja) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Atención: Debes abrir la caja antes de crear servicios.'),
      ...
      if (!mounted) return;
      setState(() {
        _error = 'Error al cargar los recursos iniciales del servicio';
        _loadingInitial = false;
      });
    }
  }
  ...
  Map<String, double> _calculateTotals() {
    final double roomPrice = _selectedRoom != null ? double.tryParse(_selectedRoom['precio']?.toString() ?? '0') ?? 0.0 : 0.0;
    final int minutes = _selectedRoom != null ? int.tryParse(_selectedRoom['tiempo']?.toString() ?? '0') ?? 0 : 0;
    
    final double inputPrecioServicio = double.tryParse(_precioServicioController.text.replaceAll('.', '')) ?? 0.0;
    
    final int numAnfitrionas = _selectedHostesses.isEmpty ? 1 : _selectedHostesses.length;
    final int numClientes = _selectedClients.isEmpty ? 1 : _selectedClients.length;

    final int multiplicadorTiempo = minutes == 60 ? 2 : 1;
    final int multiplicadorServicio = numAnfitrionas;
    
    final double comisionAnfitriona = _selectedRoom != null ? double.tryParse(_selectedRoom['comision_anfitriona']?.toString() ?? '0') ?? 0.0 : 0.0;
    final bool tieneComision = comisionAnfitriona > 0;
    
    final double multiplicadorHabitacion = tieneComision ? 1.0 : (numAnfitrionas > numClientes ? numAnfitrionas : numClientes).toDouble();

    final double precioServicioActual = inputPrecioServicio * multiplicadorTiempo * multiplicadorServicio;
    final double precioHabitacionActual = roomPrice * multiplicadorTiempo * multiplicadorHabitacion;
    ...
    double currentTotal = precioServicioActual + precioHabitacionActual + calculatedIva;
    ...
    final double comisionPorAnfitriona = (tieneComision && _selectedHostesses.isNotEmpty)
  ...
  void _updateClientSaldo(dynamic clientId, double nuevoSaldo) {
    setState(() {
      ...
      if (_selectedClients.isNotEmpty) {
        final primaryId = (_selectedClients.first['id_cliente'] ?? _selectedClients.first['id']).toString();
  ...
  Future<void> _handleLoadBalance(dynamic clientData) async {
    ...
    final double amount = double.tryParse(_balanceAmountController.text.replaceAll('.', '')) ?? 0;
    if (amount <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Ingrese un monto válido a cargar'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    try {
      final res = await api.dio.post('/clients/prepago', data: {
        'cliente_id': clientId,
        'monto': amount,
        'tipo': 'CARGA'
      });
      ...
      if (res.data != null && res.data['success'] == true) {
        final double nuevoSaldo = double.tryParse(res.data['data']['nuevo_saldo']?.toString() ?? '') ?? 0.0;
        _updateClientSaldo(clientId, nuevoSaldo);
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Saldo cargado correctamente. Nuevo Saldo: ${_formatCurrency(nuevoSaldo)}'), backgroundColor: Colors.green),
        );
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error al procesar la carga de saldo'), backgroundColor: Colors.redAccent),
      );
    } finally {
        ...
        return AlertDialog(
            ...
            children: [
              ...
              TextField(
                ...
                onChanged: (val) {
                  ...
                  final formatted = NumberFormat.currency(locale: 'es_CL', symbol: '', decimalDigits: 0)
                      .format(double.tryParse(clean))
                      .trim();
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              child: Text('Cargar Saldo', style: GoogleFonts.inter(fontWeight: FontWeight.bold)),
            ),
          ],
        );
      },
  ...
  Future<void> _submitServicio() async {
    if (!_cajaAbierta) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Debe abrir la caja antes de registrar servicios.'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    if (_selectedRoom == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Seleccione una habitación o mesa'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    if (_selectedHostesses.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Seleccione al menos una anfitriona'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    final double inputPrecioServicio = double.tryParse(_precioServicioController.text.replaceAll('.', '')) ?? 0.0;
    final double comisionAnfitriona = double.tryParse(_selectedRoom['comision_anfitriona']?.toString() ?? '0') ?? 0.0;
    final bool hasAnfitrionaComision = comisionAnfitriona > 0;

    // Mixed payment validation
    if (_paymentMethod == 'mixto') {
      final double totalIngresado = _pagosMixtos.fold(0.0, (sum, item) => sum + (item['monto'] as double));
      if ((totalIngresado - total).abs() > 1.0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('El monto total ingresado (\$${_formatCurrency(totalIngresado)}) debe coincidir con el total (\$${_formatCurrency(total)})'),
      ...
      if (_pagosMixtos.length < 2) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Seleccione al menos 2 métodos de pago para pago mixto'), backgroundColor: Colors.redAccent),
        );
        return;
      }
    }
    ...
    final primaryClient = _selectedClients.isNotEmpty ? _selectedClients.first : null;
    final double saldoPrepago = primaryClient != null ? double.tryParse(primaryClient['saldo']?.toString() ?? '0') ?? 0.0 : 0.0;

    if (_paymentMethod == 'prepago') {
      if (primaryClient == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Seleccione un cliente para realizar el pago con prepago'), backgroundColor: Colors.redAccent),
        );
        return;
      }
      if (saldoPrepago < total && _metodoPagoAdicional.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('El saldo es insuficiente. Seleccione un método de pago adicional.'), backgroundColor: Colors.redAccent),
        );
        return;
      }
    }
    ...
    if (_paymentMethod == 'mixto') {
      final prepagoItem = _pagosMixtos.firstWhere((p) => p['metodo'] == 'prepago', orElse: () => {});
      if (prepagoItem.isNotEmpty) {
        final double prepagoMonto = prepagoItem['monto'] as double;
        if (prepagoMonto > saldoPrepago) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('El monto de prepago ingresado excede el saldo disponible del cliente.'), backgroundColor: Colors.redAccent),
          );
          return;
        }
      }
    ...
    try {
      final List<String> hostessesIds = _selectedHostesses.map((h) => (h['id_usuario'] ?? h['id']).toString()).toList();
      final List<String> clientsIds = _selectedClients.map((c) => (c['id_cliente'] ?? c['id']).toString()).toList();
      final String? mainClientId = primaryClient != null ? (primaryClient['id_cliente'] ?? primaryClient['id']).toString() : null;

      final Map<String, dynamic> payload = {
        'codigo': _generateCode(),
        'cliente_id': mainClientId,
        'clientes': clientsIds,
        'habitacion_id': (_selectedRoom['id_habitacion'] ?? _selectedRoom['id']).toString(),
      ...
      if (response.data != null && response.data['success'] == true) {
        ...
        final msg = response.data?['message'] ?? 'Error al registrar el servicio';
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error: $msg'), backgroundColor: Colors.redAccent),
        );
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error de conexión al registrar servicio'), backgroundColor: Colors.redAccent),
      );
    } finally {
      if (mounted) setState(() => _submitting = false);
    }
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isSelected = _selectedHostesses.any((h) => (h['id_usuario'] ?? h['id']).toString() == hostId.toString());

                        return CheckboxListTile(
                          value: isSelected,
                          title: Text(host['nick'] ?? host['nombre'] ?? 'Anfitriona'),
                          subtitle: Text('Estado: ${host['status'] ?? 'Activa'}'),
                          ...
                          onChanged: (val) {
                            setModalState(() {
                              if (val == true) {
                                ...
                                _selectedHostesses.removeWhere((h) => (h['id_usuario'] ?? h['id']).toString() == hostId.toString());
                              }
                            });
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isSelected = _selectedClients.any((c) => (c['id_cliente'] ?? c['id']).toString() == clientId.toString());

                        return CheckboxListTile(
                          value: isSelected,
                          title: Text('${client['nombre'] ?? ''} ${client['apellido'] ?? ''}'.trim()),
                          subtitle: Text('Saldo: ${_formatCurrency(double.tryParse(client['saldo']?.toString() ?? '0') ?? 0.0)}'),
                          activeColor: AppTheme.primaryColor,
                          onChanged: (val) {
                            setModalState(() {
                              if (val == true) {
                                ...
                                _selectedClients.removeWhere((c) => (c['id_cliente'] ?? c['id']).toString() == clientId.toString());
                              }
                            });
                            setState(() {}); // refresh outer screen
                          },
      ...
      builder: (context) {
        ...
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              backgroundColor: isDark ? AppTheme.darkSurfaceColor : AppTheme.lightSurfaceColor,
              title: Text('Agregar Medio de Pago', style: GoogleFonts.inter(fontWeight: FontWeight.bold, fontSize: 16)),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    if (val <= 0) return;
                    setState(() {
                      _pagosMixtos.removeWhere((p) => p['metodo'] == selectedMethod);
                      _pagosMixtos.add({
                  ...
                  child: Text('Agregar', style: GoogleFonts.inter(fontWeight: FontWeight.bold)),
                ),
              ],
            );
          },
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: DropdownButtonFormField<dynamic>(
                        ...
                        hint: Text('Seleccionar Habitación / Mesa (Requerido)', style: GoogleFonts.inter(fontSize: 13)),
                        decoration: const InputDecoration(
                        ...
                        items: _habitaciones.map((room) {
                          return DropdownMenuItem<dynamic>(
                            value: room,
                            child: Text('${room['nombre']} - (${room['tiempo'] ?? 0} mins) - \$${(room['precio'] as double).toStringAsFixed(0)}', style: const TextStyle(fontSize: 13)),
                          );
                        }).toList(),
                        onChanged: (val) {
                          setState(() {
                                ...
                                children: [
                                  ...
                                  Text(
                                    _selectedHostesses.isEmpty
                                        ...
                                        : _selectedHostesses.map((h) => h['nick'] ?? h['nombre'] ?? '').join(', '),
                                ...
                                children: [
                                  ...
                                  Text(
                                    _selectedClients.isEmpty
                                        ...
                                        : _selectedClients.map((c) => '${c['nombre'] ?? ''} ${c['apellido'] ?? ''}'.trim()).join(', '),
                                    style: GoogleFonts.inter(fontSize: 12, color: isDark ? AppTheme.darkTextSecondary : AppTheme.lightTextSecondary),
                                    maxLines: 2,
                                    overflow: TextOverflow.ellipsis,
                                  ),
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () => _showCargarSaldoModal(primaryClient),
                              child: Text('CARGAR', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.bold)),
                            ),
                          ],
                        ),
                      ),
                    ...
                    Container(
                      ...
                      child: DropdownButtonFormField<String>(
                        ...
                        items: [
                          ...
                          const DropdownMenuItem(value: 'tarjeta', child: Text('Tarjeta')),
                          const DropdownMenuItem(value: 'transferencia', child: Text('Transferencia')),
                          if (primaryClient != null)
                            const DropdownMenuItem(value: 'prepago', child: Text('Prepago (Saldo Cliente)')),
                          const DropdownMenuItem(value: 'mixto', child: Text('Mixto (Varios Métodos)')),
                        ],
                        onChanged: (val) {
                          if (val != null) {
                            setState(() {
                              ...
                              if (val == 'mixto' && primaryClient != null && saldoPrepago > 0) {
                                _pagosMixtos.add({
                                  'metodo': 'prepago',
                                  'monto': saldoPrepago,
                                });
                    ...
                    if (_paymentMethod == 'prepago' && saldoPrepago < total) ...[
                      const SizedBox(height: 12),
                      Text(
                      ...
                      Container(
                        ...
                        child: DropdownButtonFormField<String>(
                          initialValue: _metodoPagoAdicional.isEmpty ? null : _metodoPagoAdicional,
                              ...
                              ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return ListTile(
                                    ...
                                    title: Text(item['metodo'].toString().toUpperCase(), style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.bold)),
                                    trailing: Row(
                                      mainAxisSize: MainAxisSize.min,
                                      children: [
                                        Text(_formatCurrency(item['monto'] as double), style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.bold)),
                                        IconButton(
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildSummaryRow(isDark, 'Habitación / Mesa', _formatCurrency(roomPriceActual)),
                          _buildSummaryRow(isDark, 'Servicio Temporal', _formatCurrency(servicePriceActual)),
                          if (iva > 0)
                            _buildSummaryRow(isDark, 'Recargo de Tarjeta (IVA/Com.)', _formatCurrency(iva), color: Colors.orange),
                          const Divider(height: 20),
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text(
                                'TOTAL NETO',
                                style: GoogleFonts.inter(fontWeight: FontWeight.bold, fontSize: 15, color: AppTheme.primaryColor),
                              ),
                              Text(
                                _formatCurrency(total),
                                style: GoogleFonts.inter(fontWeight: FontWeight.w900, fontSize: 20, color: Colors.green),
                              ),
                            ],
                          ),
                        ],
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: _submitting ? null : _submitServicio,
                        child: _submitting
                            ? const SizedBox(
                                height: 20,
                                width: 20,
