...
class _HorasExtrasScreenState extends ConsumerState<HorasExtrasScreen> {
    ...
    try {
      ...
      if (isManual) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Horas extras actualizadas'),
  ...
  Widget build(BuildContext context) {
    ...
    // Calculations
    final pendientes = _horasExtras.where((a) => a['estado']?.toString() == '1');
    final double totalPendiente = pendientes.fold(0.0, (sum, item) => sum + (double.tryParse(item['total']?.toString() ?? item['monto']?.toString() ?? '0') ?? 0.0));
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _buildFilterButton('all', 'Todas (${_horasExtras.length})', isDark),
                      const SizedBox(width: 8),
                      _buildFilterButton('pendiente', 'Pendientes (${_horasExtras.where((a) => a['estado']?.toString() == '1').length})', isDark),
                      const SizedBox(width: 8),
                      _buildFilterButton('pagado', 'Cobradas (${_horasExtras.where((a) => a['estado']?.toString() == '0').length})', isDark),
                    ],
                  ),
                  const SizedBox(height: 16),
  ...
  Widget _buildOvertimeCard(dynamic item, int index, bool isDark) {
    ...
    final double amount = double.tryParse(item['total']?.toString() ?? item['monto']?.toString() ?? '0') ?? 0.0;
